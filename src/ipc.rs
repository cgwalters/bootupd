...
pub(crate) enum DaemonToClientReply<T: serde::Deserialize<'static>> {
    Success(T),
    Failure(String),
}
...
impl ClientToDaemonConnection {
    ...
    pub(crate) fn send<T: Deserialize<'static>>(&mut self, opt: &crate::Opt) -> Result<DaemonToClientReply<T>> {
        {
